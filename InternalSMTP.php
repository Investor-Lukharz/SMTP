    ...
    if(isset($_POST["generate_otp"])){
        ...
        $contact = trim($_POST["contact"]);
            ...
            // Define action.
                ...
                foreach($session_rooms as $keys => $room_id ){
                    if($room_id == $otp){
...
<head>
    ...
    <link rel="stylesheet" href="smtpstyle.css">
</head>
    <body>
        <header>
            <img src="img/icon.jpg.jpg" width="100px" height="100px" class="image">
                <div class="brand">
                    <div class="name">
            ...
            <!-- Show Room for user. -->
                <div class="indicator">
                    ...
                    <div class="step active">3</div> 
                        ...
                        <div class="card">
                            <!-- Get the id and room saved in session and check if it match the current user id  -->
                            <?php foreach($session_rooms as $keys => $room_id): ?>
                                <!-- Check if the id match the one saved in the session  -->
                                <?php if($room_id == $_SESSION["User_Data"]["Room Id"]) : ?>
                                    <!-- if it match get such room and access it from the $rooms array. -->
                                    <?php foreach($rooms[$keys]  as $features ): ?>
                                        <div class="thumbnail">
                                            <img class="thumb_img" src="<?php echo $rooms[$keys]["image"] ; ?>" width="100%" height="100%" alt="Room">     
                                        </div>
                                        <h3 >Your room id : <?php echo $room_id ?></h3>
                                        <ul>
                                           <?php foreach($rooms[$keys]["Features"] as $spec):?>  
                                                <li><?php echo $spec ; ?></li>
                                            <?php endforeach ?>
                                        </ul>
                                        <h3><?php echo $rooms[$keys]["Prices"] ;?></h3>
                                        <?php break; ?>
                                    <?php endforeach ?>
                                <?php else: ?>
                                    <?php continue ; ?>
                            ...
                            <?php endforeach ?>
                            <form method="POST">
                                    <button type="submit" class="submit" name="book"> Click to Book</button>
                            </form>
                        </div>
                         <form method="POST">
            ...
            <!-- Form Registration -->
                <div class="indicator">
                    ...
                    <h3 class="heading">Provide Your Detail to Book Room</h3>
